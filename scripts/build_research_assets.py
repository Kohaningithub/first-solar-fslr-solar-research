...
def save_tweet_valuation_card(
    ...
    draw.text((92, 112), "估值：base case 已低于当前价", font=title_font, fill="#10231F")
    draw.text((96, 182), "这不是明显低估；bull case 要靠 45X、backlog ASP 和 2027-2029 收入桥兑现", font=sub_font, fill="#51615A")
...
def main() -> None:
    ...
    research_path = [
        {"key_question": "Is FSLR fairly valued, overvalued, or still underappreciated given 2026 EBITDA power and policy-adjusted risk?", "why_it_matters": "当前股价已接近基本面定价，继续上行要靠 earnings power 兑现、政策折现下降或估值倍数扩张。", "data_to_track": "FY2026 EBITDA run-rate, EV/FY2026 EBITDA, TTM P/E, net cash, policy-adjusted multiple.", "threshold_or_signpost": "EV/FY2026 EBITDA <9x 且 backlog ASP 稳定更适合分批；>11-12x 需要明确政策/订单催化。", "current_read": f"At ~{key_metrics['ev_ebitda_fy2026_mid']:.1f}x FY2026 EBITDA and ~{key_metrics['pe_ttm_company_basis']:.1f}x TTM P/E, FSLR is no longer a deep-value solar trade; further upside needs gross margin strength, backlog conversion and policy clarity.", "source_id": "S1;S3;S11;S12"},
...
FSLR 的核心矛盾不是“太阳能行业景气度是否好”，而是美国本土薄膜组件产能能不能在政策保护期内，把 45X、贸易壁垒和数据中心/公用事业需求转成可持续现金流。公司当前基本面很强：Q1 2026 收入 $1.044bn、毛利率 46.6%、调整后 EBITDA $520mn、净现金约 $2.0bn，backlog 47.9GW/$14.4bn。问题在于股价已接近基本面定价，按公司口径估算 EV/FY2026 EBITDA midpoint 约 {key_metrics['ev_ebitda_fy2026_mid']:.1f}x，base case 并不显示明显低估。
...
情景上，熊市情景用 $2.3bn EBITDA 和 8x EV/EBITDA，隐含股价约 ${scenarios[0]['implied_price_usd']:.0f}；基本情景用 $2.7bn 和 10x，约 ${scenarios[1]['implied_price_usd']:.0f}；牛市情景用 $3.2bn 和 12x，约 ${scenarios[2]['implied_price_usd']:.0f}。当前价格已经高于 base case，说明市场已经反映一部分 45X 和 backlog 可见度，继续上涨需要新增订单 ASP、backlog 兑现、毛利率稳定和政策确定性共同支持。
...
"""
    ...
    post = f"""FSLR（First Solar）我现在的看法是：中期看好公司核心价值，但不看好当前位置无验证追高。
...
因为好公司不等于好买点。用当前股价约 ${key_metrics['current_price_usd']:.2f}、Q1 diluted shares 107.6mn、净现金约 $2.0bn 粗算，公司口径 EV 约 ${key_metrics['ev_usd_bn_company_basis']:.1f}bn。对 FY2026 adjusted EBITDA midpoint $2.7bn，是大约 {key_metrics['ev_ebitda_fy2026_mid']:.1f}x EV/EBITDA。base case implied price 低于当前股价，说明市场已经把一部分 45X 和 backlog 可见度计进去了。
...
所以我的结论是：中期趋势偏正面，但短期趋势更像事件驱动和估值消化。若 45X 规则稳定、Section 232/AD-CVD/TOPCon 继续抬高进口竞争成本，同时新增订单 ASP 稳住，FSLR 有机会从基本情景往牛市情景走。反过来，如果政策折现变大、项目成本伤害需求，或者 2027-2029 新单接不上，股价更容易横盘甚至回撤。
...
我的操作框架很简单：当前股价已接近基本面定价，继续持有更依赖 earnings power 兑现；新资金宜等待回撤、backlog ASP 改善、45X 政策确定性提高，或 2027-2029 收入桥更加清晰。如果 EV/FY2026 EBITDA 回到 9x 附近，同时新增订单价格稳，我会觉得性价比更好。如果股价先涨到牛市情景，但没有 backlog ASP、45X 稳定性和 2027-2029 新单支撑，那就更像事件交易，不是舒服的长期加仓点。

一句话：FSLR 不是纯粹低估标的，而是高质量但政策敏感的 earnings compounder。接下来真正要盯的不是太阳能新闻热度，而是新增 bookings ASP、backlog conversion、45X cash realization、OCF-capex 和贸易政策落地。
"""
    ...
    save_card(
        ...
        [
            ...
            "投资方向：接近基本面定价；持有看兑现，新资金等回撤或订单/政策验证",
        ],
        font_path,
        "#0E7C66",
    )
    ...
    save_tweet_card(
        ...
        "FSLR：好公司，但不是便宜买点",
        "当前股价已接近基本面定价；继续持有更依赖 earnings power 兑现。",
        [
            ("核心价值", "美国本土 CdTe 薄膜组件产能，叠加政策信用、净现金和已签 backlog。"),
            ("当前判断", f"EV/FY2026 EBITDA 约 {key_metrics['ev_ebitda_fy2026_mid']:.1f}x，base case implied price 低于 current。"),
            ("投资方向", "已有仓位看兑现；新资金等回撤、backlog ASP 改善或 45X 政策确定性提高。"),
            ("关键变量", "新增 bookings ASP、backlog conversion、45X cash realization、OCF-capex。"),
        ],
        font_path,
        "#0E7C66",
    )
    ...
    save_tweet_card(
        ...
        [
            ...
            ("结论", "FSLR 的溢价有理由，但已不小；后续要靠订单和政策继续兑现。"),
        ],
        font_path,
        "#6E5AA8",
    )
    ...
    save_tweet_card(
        ...
        "下一步验证：只看这几组数据",
        "FSLR 后续上涨需要新增证据，而不是只重复 backlog 很大。",
        [
            ("订单", "New bookings GW、new bookings ASP、delivery years、customer concentration。"),
            ("政策", "45X cash realization、IRS/Treasury 规则、FEOC/PFE 限制、Section 232 进展。"),
            ("现金流", "OCF-capex 是否恢复；2025 FCF 强，Q1 2026 OCF 为负需要 Q2/Q3 验证。"),
            ("估值纪律", f"Base case vs current 约 {current_vs_base:.1%}；回到 9x EV/FY2026 EBITDA 附近更有安全垫。"),
        ],
        font_path,
        "#10231F",
    )
